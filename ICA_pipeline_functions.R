# pipeline_sorted_ids.R
# Basic, readable pipeline with:
# - probeset/ENTREZ/SYMBOL detection
# - mapping to CHR/BP and genome sorting
# - RANDOM keep-one for duplicate gene IDs (even without mapping)
# - PC1 "same-sign" rule + exports
# - ICA, optional GSEA, optional associations, optional survival tree

suppressPackageStartupMessages({
  library(data.table)
  library(fastICA)
  library(survival)
  # Optional: MST (for survival tree), pheatmap (for heatmaps)
})
...
# -----------------------
# EDA (defensive against duplicate rownames in the *output* tables)
# -----------------------
summarize_expression_metrics <- function(expr, out_prefix) {
  ...
  # NEW: make output row labels unique if needed (does not change expr)
  rn <- rownames(expr)
  rn_unique <- if (anyDuplicated(rn)) {
    message("Found duplicated rownames in expr; EDA output row labels will be made unique.")
    make.unique(rn, sep = "_dup")
  } else rn
  
  safe <- function(f) function(x) f(x, na.rm = TRUE)
  per_gene <- data.frame(
    mean   = apply(expr, 1, safe(mean)),
    median = apply(expr, 1, safe(median)),
    max    = apply(expr, 1, safe(max)),
    min    = apply(expr, 1, safe(min)),
    n_na   = apply(expr, 1, function(x) sum(is.na(x))),
    iqr    = apply(expr, 1, function(x) IQR(x, na.rm = TRUE)),
    row.names = rn_unique
  )
  per_sample <- data.frame(
    mean   = apply(expr, 2, safe(mean)),
    median = apply(expr, 2, safe(median)),
    max    = apply(expr, 2, safe(max)),
    min    = apply(expr, 2, safe(min)),
...
# -----------------------
# NEW: ID mapping + genome sorting (random keep-one duplicates)
# -----------------------
map_and_sort_genes <- function(expr, mapping = NULL,
  ...
  # --- NO MAPPING PROVIDED: dedup by rowname and return sorted (alphabetical) ---
  if (is.null(mapping)) {
  ...
  map_df <- mapping
  map_df$.__row_probe__ <- rownames(mapping)
  ...
  if (is_probeset) {
    ...
    gene_id <- if (!is.na(ent_col) && ent_col %in% colnames(msub)) {
      as.character(msub[[ent_col]])
    } else if (!is.na(sym_col) && sym_col %in% colnames(msub)) {
      as.character(msub[[sym_col]])
    } else rn
  } else if (id_guess == "ENTREZID" && !is.na(ent_col) && ent_col %in% colnames(map_df)) {
    msub <- map_df[match(rn, as.character(map_df[[ent_col]])), , drop=FALSE]
    gene_id <- rn
  } else if (id_guess == "SYMBOL" && !is.na(sym_col) && sym_col %in% colnames(map_df)) {
    msub <- map_df[match(rn, as.character(map_df[[sym_col]])), , drop=FALSE]
    gene_id <- rn
  } else {
    msub <- map_df[rep(NA_integer_, length(rn)), , drop=FALSE]
    gene_id <- rn
  }
  ...
  CHR[is.na(CHR) | CHR==""] <- "unknown"
  CHR_norm <- tolower(CHR); CHR_norm <- sub("^chr", "", CHR_norm)
  ...
  # Deduplicate: randomly keep one per gene_id
  ...
  list(expr_sorted = expr_sorted,
       gene_table  = gene_tbl_sorted,
       id_space    = guess_id_space(rownames(expr_sorted)))
}

# -----------------------
# PCA / PC1 same-sign rule
# -----------------------
...
pc1_sign_counts <- function(pca_loadings_col1) {
  s <- sign(pca_loadings_col1)
  c(pos = sum(s > 0, na.rm=TRUE),
    neg = sum(s < 0, na.rm=TRUE),
    zero = sum(s == 0, na.rm=TRUE))
}
should_remove_pc1_same_sign <- function(counts) {
  no_mix <- (counts["pos"] == 0L) || (counts["neg"] == 0L)
  has_signal <- (counts["pos"] + counts["neg"]) > 0L
  isTRUE(no_mix && has_signal)
}

# -----------------------
# ICA / IC scatter / GSEA / associations
# -----------------------
...
plot_ic_scatter_pdf <- function(IC_S, out_file = "independent_components_scatter.pdf") {
  n_comp <- ncol(IC_S)
...
read_gmt <- function(gmt_file){
  ...
  g <- list(); repeat {
    line <- readLines(con, n=1); if (!length(line)) break
...
gsea_mw_on_vector <- function(w, gene_sets, min_size=10, max_size=500){
  ...
  if (any(duplicated(names(w)))) {
    byid <- split(w, names(w))
    w <- vapply(byid, function(v) v[which.max(abs(v))], numeric(1))
  }
...
gsea_matrix <- function(IC_S, gmt_file, out_pdf, out_csv){
  ...
  for (j in seq_len(ncol(IC_S))) {
    w <- IC_S[, j]; names(w) <- rownames(IC_S)
    v <- gsea_mw_on_vector(w, gs); col_list[[j]] <- v; all_names <- union(all_names, names(v))
  }
  M <- matrix(NA_real_, nrow=length(all_names), ncol=length(col_list),
              dimnames=list(all_names, comp))
...
assoc_matrix <- function(mixing_matrix, annot){
  X <- as.matrix(mixing_matrix)  # samples x ICs
...
plot_assoc_heatmap <- function(M_logp, out_pdf = "associations_heatmap.pdf", title = "-log10(p) associations") {
  if (requireNamespace("pheatmap", quietly = TRUE)) {
...
# MAIN
# -----------------------
run_pipeline_sorted_ids <- function(
    expr_path,
    mapping_path = NULL,          # optional; used for probeset/SYMBOL mapping + CHR/BP sorting
    annot_path   = NULL,          # optional; needed for associations/survival
    gmt_path     = NULL,          # optional; GSEA runs only if IDs are ENTREZ/SYMBOL
    out_dir      = getwd(),
    explained_variance_aim = 0.70,
    survival_time_col  = NULL,    # REQUIRED to run survival tree
    survival_event_col = NULL     # REQUIRED to run survival tree
) {
  ...
  # ---- Load
  expr0   <- ensure_numeric(read_table(expr_path))          # rows = probes/genes, cols = samples
  ...
  # ---- Map IDs + sort by genome (and RANDOM keep-one duplicates)
  ...
  # Align samples if annotation provided
  if (!is.null(annot)) {
    common_samp <- intersect(colnames(expr), rownames(annot))
    if (length(common_samp)) {
      expr  <- expr[, common_samp, drop=FALSE]
      annot <- annot[common_samp, , drop=FALSE]
    } else {
      message("No overlapping samples between expression and annotation; associations/survival will be skipped.")
      annot <- NULL
    }
  }
  ...
  # ---- PCA + same-sign decision; export PC1 summary/loadings/scores
  ...
  pca_info <- data.frame(
    metric   = c("PC1_variance_explained",
                 "PC1_loading_pos_count",
                 "PC1_loading_neg_count",
                 "PC1_loading_zero_count",
                 "Remove_PC1_by_same_sign_rule"),
    value    = c(round(pc1_var, 6),
                 pc1_counts["pos"], pc1_counts["neg"], pc1_counts["zero"],
                 remove_pc1_flag),
    stringsAsFactors = FALSE
  )
  write.table(pca_info, file = file.path(out_dir, "Results/pca_summary.tsv"),
              sep = "\t", quote = FALSE, row.names = FALSE)
  
  pc1_loadings <- loadings_mat[, 1, drop=FALSE]; colnames(pc1_loadings) <- "PC1_loading"
  write.table(pc1_loadings, file = file.path(out_dir, "Results/pc1_loadings.txt"),
              sep = "\t", quote = FALSE)
  pc1_scores <- pca1$scores[, 1, drop=FALSE]; colnames(pc1_scores) <- "PC1_score"
  write.table(pc1_scores, file = file.path(out_dir, "Results/pc1_scores.txt"),
              sep = "\t", quote = FALSE)
  
  # ---- ICA (on PC1-removed if rule triggers, else original)
  ...
  # ---- GSEA (only if ENTREZ/SYMBOL + GMT provided)
  if (!is.null(gmt_path) && id_space %in% c("ENTREZID","SYMBOL")) {
    ...
    message("GSEA not conducted (either GMT not provided or gene IDs not ENTREZ/SYMBOL).")
  }
  
  # ---- Associations + Survival (explicit survival columns)
  if (!is.null(annot)) {
    ...
    if (!is.null(survival_time_col) && !is.null(survival_event_col) &&
      ...
      message("Survival tree skipped: provide valid 'survival_time_col' and 'survival_event_col' present in annotation (and have MST installed).")
    }
  } else {
    message("No sample annotations provided → Skipping associations and survival.")
  }
  
  invisible(list(
    expression_sorted = expr,
    gene_table_sorted = gene_tbl_sorted,
    id_space = id_space,
